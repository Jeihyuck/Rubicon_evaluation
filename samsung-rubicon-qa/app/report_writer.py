...
def _build_summary(run_results: list[RunResult]) -> str:
    ...
    lines = [
        ...
        "",
        "결과 확인 우선순위: `reports/latest_conversation.md` -> `reports/latest_results.json` -> `reports/latest_results.csv` -> `reports/summary.md` -> `artifacts/chatbox/*_before_send.png` 및 `artifacts/chatbox/*_after_answer.png`",
        "`before_send`는 입력 성공 증거, `after_send`는 제출 효과 증거, `after_answer`는 실제 답변 증거다.",
        "",
        "## 집계",
        "",
        f"- 총 케이스 수: {total}",
